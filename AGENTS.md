...
This branch is a Java 17 Minecraft `1.18.2` port branch. It targets two loaders:
...
- `forge/`: Forge `1.18.2` build and run configuration.
- `fabric/`: Fabric `1.18.2` build and run configuration.

Resources live in `src/main/resources`, including assets under `assets/enchantment_custom_table` and data files under `data/enchantment_custom_table`. For Minecraft `1.18.2`, use resource pack format 8 and legacy data-pack directory names such as `recipes/`, `loot_tables/`, and `tags/items/`. Custom generated-slot and floating-book sprites must be explicitly registered with the loader's pre-atlas-definition stitching API.

## Build, Test, and Development Commands

Use the Gradle wrapper from the repository root. This branch targets Java 17 bytecode; the local Gradle runtime may run on Java 21 while Gradle toolchains compile with Java 17. The wrapper is intentionally pinned to Gradle 8.8 because it is the verified common baseline for the current ForgeGradle and Fabric Loom builds and avoids a Windows Forge launcher-generation failure seen with Gradle 8.14.3. Before upgrading it, verify `:forge:build`, `:forge:prepareRunClient`, and `:forge:prepareRunClientCompile` on Windows as well as the Fabric build.

- `./gradlew :common:test` runs fast shared JVM tests.
- `./gradlew :forge:build` builds the Forge `1.18.2` jar.
- `./gradlew :fabric:build` builds the Fabric `1.18.2` jar.
...
- `enchantment_custom_table-<mod_version>-forge-mc1.18.2.jar`
- `enchantment_custom_table-<mod_version>-fabric-mc1.18.2.jar`
