...
E02/E06 及严格配置扩展，解除其中 4 项历史阻塞；其他版本的 Shift 结果尚未更新。
...
- Computer Use 接口没有带修饰键的鼠标点击接口；单独发送 `Shift_L` 被拒绝。
  下述独立原生输入辅助程序已完成 1.21.1 双端的 Shift 取书/插书实测；其他版本仍待验证，
  不能以普通点击结果代替。
...
`macos_shift_click.swift` 是用户授权的测试输入辅助程序，只向专用 1.21.1 客户端发送
...
程序仅接受本工具生成的 Fabric/NeoForge 1.21.1 bundle identity，检查目标窗口、坐标、
...
该辅助程序目前明确限制为 1.21.1，不能据此宣称解决 26.3 的 SDL 坐标问题。
