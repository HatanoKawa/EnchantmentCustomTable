...
- The [tiered matrix](../../tools/gui-validation/tiered-matrix.md) covers all 15 versions on both loaders: nine full-suite versions and six quick-suite versions. The [third-round results](../../tools/gui-validation/round3-2026-09-22.md), completed on 2026-09-23, record successful builds for all 30 targets, 548 fresh GUI passes, 52 explicitly reused round-two passes, 36 unsupported Shift-click cases blocked, and no pending cases. Three Mac lock interruptions were resolved by manual unlock; all original config/options files are restored and temporary per-client wake processes have ended. The quick suite does not include configuration combinations.
- Third-round migration observations found old table inventory/component loading failures when upgrading copied 1.21.1 worlds. These remain open and separate from same-version persistence passes; a successful menu regression does not establish safe world upgrades.
